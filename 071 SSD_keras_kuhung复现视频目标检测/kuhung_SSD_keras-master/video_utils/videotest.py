...
class VideoTest(object):
	...
	#skv=skvideo.io.VideoCapture(video_path)#elesun
	skv=skvideo.io.vread(video_path)#elesun add
	T, height, width, C = skv.shape
        #vidar = float(skv.width)/skv.height
	#vidaz = float(skv.height)/self.input_shape[0]
	vidar = float(width)/height
	vidaz = float(height)/self.input_shape[0]
