...
print('bostondata_X:',x.shape)# (506, 13)
# print(x[::100])
print('bostonpric_Y:',y.shape)
# print(y[::100])
# 数据标准化
...
def get_batch_boston():
    ...
    print('date=', BATCH_START, BATCH_START + TIME_STEPS * BATCH_SIZE)
...
def get_batch():
    ...
    # plt.show()
    print('before add aixs:',seq.shape)
    print(  seq[:2])
    print('after  add aixs:',seq[:, :, np.newaxis].shape)
    print(seq[:2])
    # returned seq, res and xs: shape (batch, step, input)
    #np.newaxis 用来增加一个维度 变为三个维度，第三个维度将用来存上一批样本的状态
...
if __name__ == '__main__':
    ...
    print('real',train_y.flatten().shape)
    ...
    line3,=axes.plot(range(100), train_y.flatten()[ - 100:], 'r',label='real')
