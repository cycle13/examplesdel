...
class PricePredictor:
    ...
    def loadData(self):
        ...
        while line:
            line = fp.readline()
            data = line.split(" ")
            if (len(data) != 7) or ('built' in data[0]) or ('*' in data[2]) or ('车位' in data[4].encode('utf-8')) or ('平米' not in data[5].encode('utf-8')):
                continue
            print line
        ...
        print 'self.sortedChargeList[-1]=',self.sortedChargeList[-1]
        print 'len(self.sortedChargeList)',len(self.sortedChargeList)
            ...
            for i in range(10000):
                ...
                while end < len(self.train_x):
                    ...
                    if step % 100 == 0:
                        ...
                        print(saver.save(sess, './model/stock.model'))
                    step += 1
    ...
    def prediction(self):
        ...
        with tf.Session() as sess:
            ...
            plt.plot(list(range(len(self.normalizeData), len(self.normalizeData) + len(predict))), true_price,label='predict', color='r')
            print 'len(self.normalizeData)',len(self.normalizeData)
            print 'len(predict)',len(predict)
            plt.xlabel("predict future num %d(length) from %d"%(len(predict),len(self.normalizeData)))
            plt.ylabel("price(W RMB)")
            plt.title("housing price prediction of ChengDu")
            plt.legend()
            plt.show()
...
# 模型训练
#predictor.trainLstm()#not both use with predictor.prediction()
# 预测－预测前需要先完成模型训练
predictor.prediction()#not both use with predictor.trainLstm
